...
jQuery.fn.dataTable.Api.register('MakeCellsEditable()', function (settings) {
    ...
    if (table != null) {
        ...
        $(table.body()).on('click', 'td', function () {
            ...
            curCol = currentColumnIndex;
            curRow = table.cell(this).index().row;

            // DETERMINE WHAT COLUMNS CANNOT BE EDITED
        ...
        $(table.body()).on('keydown', function (ev) {
            if (ev.shiftKey && ev.keyCode == 9) {
                ev.preventDefault();
                curCol = curCol-1;
                var currentColumnIndex = curCol;

                // DETERMINE WHAT COLUMNS CANNOT BE EDITED
                if ((settings.columns && settings.columns.indexOf(currentColumnIndex) == -1) || (!settings.columns)) {
                    var cell = table.cell(curRow, curCol).node();
                    var oldValue = table.cell(curRow, curCol).data();
                    // Sanitize value
                    oldValue = sanitizeCellValue(oldValue);
